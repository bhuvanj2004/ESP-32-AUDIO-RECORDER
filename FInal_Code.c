...
// PIN DDEFINITIONS FOR SD CARD READER
#define SD_CS_PIN 13
...
#define SAMPLE_RATE 48000  // CD Quality is 44100, 48000 GIVES BETTER AUDIO QUALITY
#define I2S_NUM I2S_NUM_0
#define WAVE_HEADER_SIZE 44 // WAV HEADER SIZE IS 44 BYTES
...
void setup() {
  ...
  if (!SD.begin(SD_CS_PIN)) {
    Serial.println("SD Card not Initialize!"); //Checks SD card present or not
    while (1) {
      digitalWrite(LED_PIN, HIGH);
      delay(100);
      digitalWrite(LED_PIN, LOW);
      delay(100); //BLINKS LED IF SD CARD NOT PRESENT
    }
  }
  ...
  i2s_set_pin(I2S_NUM, &pin_config); //Configure GPIOS to I2S signals
  Serial.println("I2S pins configured");
  i2s_zero_dma_buffer(I2S_NUM); //Clear DMA buffers remove garbage values

  Serial.println("System is Ready. Connect pin 14 to 3.3V for Recording.");
}

void loop() {

  // Start recording when GPIO goes HIGH (connected to 3V3)
  if (digitalRead(BUTTON_PIN) == HIGH && !isRecording) {
    delay(50); // small debounce
    startRecording();
  }

  // Stop recording when GPIO goes LOW
...
void stopRecording() {
  Serial.println("Recording is Stopped. Please remove SD card. ");
  isRecording = false;
